...
const WILAYAS: SelectOption[] = [
  ...
  { value: '06', label: 'Bejaia' },
  { value: '07', label: 'Biskra' },
  { value: '08', label: 'Bechar' },
...
@Component({
  ...
  template: `
    <div *transloco="let t" class="relative flex min-h-screen overflow-hidden">
      ...
      <div class="hidden lg:flex lg:w-5/12 relative items-center justify-center p-12">
        @if (estimate()) {
          <div class="relative z-10 max-w-md animate-fade-up">
            ...
            <h2 class="text-2xl font-bold text-slate-800 mb-2">{{ t('auth.register.orderReady') }}</h2>
            <p class="text-slate-500 mb-6">{{ t('auth.register.registerToContinue') }}</p>
          ...
          <div class="relative z-10 max-w-md animate-fade-up">
            ...
            <h2 class="text-3xl font-bold text-slate-800 leading-tight mb-4">
              {{ t('auth.registerClient.brandTitle') }}
            </h2>
            <p class="text-lg text-slate-500 leading-relaxed mb-10">
              {{ t('auth.registerClient.brandSubtitle') }}
            </p>
              ...
              @for (benefit of benefits; track benefit) {
                <div class="flex items-start gap-3">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-emerald-100">
                    <svg class="w-4.5 h-4.5 text-emerald-600" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                    </svg>
                  </div>
                  <div>
                    <p class="text-sm font-semibold text-slate-800">{{ t('auth.registerClient.benefit' + benefit + '.title') }}</p>
                    <p class="text-sm text-slate-500">{{ t('auth.registerClient.benefit' + benefit + '.desc') }}</p>
                  </div>
                </div>
              }
            </div>
      ...
      <!-- Right form panel -->
      <div class="flex w-full lg:w-7/12 items-center justify-center p-4 sm:p-8 relative z-10 pt-20">
        <div class="glass-card w-full max-w-[520px] rounded-3xl border border-white/60 p-8 sm:p-10 shadow-2xl shadow-blue-900/5 animate-fade-up-delay">
          ...
          <!-- Header -->
          <div class="mb-8">
            <h1 class="text-2xl font-bold text-slate-900">{{ t('auth.registerClient.title') }}</h1>
            <p class="mt-2 text-sm text-slate-500">{{ t('auth.registerClient.subtitle') }}</p>
          </div>
          ...
          <div class="flex items-center gap-2 mb-8">
            @for (step of [1, 2]; track step) {
              ...
              >
                ...
                >
                  {{ t('auth.registerClient.step' + step) }}
                </span>
              </div>
              @if (step < 2) {
                <div class="h-0.5 flex-1 rounded-full transition-colors duration-300"
                  [class]="currentStep() > 1 ? 'bg-blue-500' : 'bg-slate-200'"></div>
              }
            }
          </div>
              ...
              <div class="space-y-4" style="animation: fade-up 0.4s ease-out both">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    @if (form.controls.firstName.touched && form.controls.firstName.invalid) {
                      <p class="mt-1.5 text-xs text-red-500 font-medium">{{ t('auth.firstNameRequired') }}</p>
                    }
                  </div>
                  <div>
                    ...
                    @if (form.controls.lastName.touched && form.controls.lastName.invalid) {
                      <p class="mt-1.5 text-xs text-red-500 font-medium">{{ t('auth.lastNameRequired') }}</p>
                    }
                  </div>
                </div>
                ...
                <div>
                  ...
                  @if (form.controls.phone.touched && form.controls.phone.invalid) {
                    <p class="mt-1.5 text-xs text-red-500 font-medium">{{ t('auth.phoneRequired') }}</p>
                  }
                </div>
                ...
                <button
                  type="button"
                  (click)="goToStep2()"
                  class="btn-glow w-full rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-3.5 text-sm font-bold text-white transition-all duration-300 hover:from-blue-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                >
                  {{ t('auth.registerClient.next') }}
                  <svg class="inline-block w-4 h-4 ms-1" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
            ...
            @if (currentStep() === 2) {
              <div class="space-y-4" style="animation: fade-up 0.4s ease-out both">
                ...
                <div>
                  ...
                  <div class="relative">
                    ...
                    <select
                      ...
                      class="select-styled input-focus-ring block w-full appearance-none rounded-xl border border-slate-200 bg-white/80 ps-11 pe-10 py-3 text-sm text-slate-900 focus:border-blue-400 focus:outline-none"
                    ...
                    >
                      ...
                      @for (w of wilayas; track w.value) {
                        <option [value]="w.value">{{ w.value }} - {{ w.label }}</option>
                      }
                    </select>
                  </div>
                  @if (form.controls.wilaya.touched && form.controls.wilaya.invalid) {
                    <p class="mt-1.5 text-xs text-red-500 font-medium">{{ t('auth.wilayaRequired') }}</p>
                  }
                </div>
                ...
                <div class="flex gap-3 pt-2">
                  ...
                  >
                    <svg class="inline-block w-4 h-4 me-1" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
                    </svg>
          ...
          <div class="space-y-3">
            ...
            >
              ...
              {{ t('auth.registerClient.registerTranslator') }}
            </a>
            <a
...
export class RegisterClientComponent {
  ...
  goToStep2(): void {
    const step1Controls = ['firstName', 'lastName', 'email', 'phone', 'password'] as const;
    let valid = true;
    for (const key of step1Controls) {
        ...
        if (savedEstimate) {
          try {
            ...
            if (Date.now() - estimate.timestamp < 3600000) {
              this.router.navigate(['/auth/verify-email'], { queryParams: { redirect: '/client/orders/new' } });
              return;
            }
          } catch { /* ignore */ }
